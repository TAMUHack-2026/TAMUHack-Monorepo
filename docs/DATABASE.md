...
* `users`: Stores user information such as their id, email, and hashed password.
  * Columns:
    ...
    * `password_hash` (this should be a SHA-256 hash of the user's password)
* `profiles`: Stores user profile data needed for the machine learning model's predictions.
  * Columns:
    ...
    * `sex` (must be either 'male' or 'female' and is case-sensitive)
    * `gender_identity` (optional)
    * `height_in` (4 digits with 2-decimal precision)
    * `weight_lbs` (5 digits with 2-decimal precision)
